...
declare module '*dist/server/entry.mjs' {
  /* Astro's node adapter in standalone mode. It wraps the listener in a record
     of its own, which is the second `server` below. */
  export const startServer: () => { server: { server: import('node:net').Server } };
}
