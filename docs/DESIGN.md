...
**macOS gets the system's menu bar, everywhere else gets the app's own gear.** macOS is the one
platform where the menu bar belongs to the system rather than to the window, so About, Settings, the
views and the editing roles hang off the real one there. Windows has no global bar at all, and the
Linux desktops that lift a window menu into their panel are not worth a third surface to draw, test
and keep honest, so both carry the gear in the rail with the same entries under it. The menu is
described once by the page, in the reader's language, and only the surface differs.
