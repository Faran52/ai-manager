...
export type PluginToggle = (plugin: InstalledPlugin, profile?: string) => Promise<void>;
...
export const usePluginToggle = (projectPath: string, reload: () => void): PluginToggle => {
  return useCallback(async (plugin: InstalledPlugin, profile?: string): Promise<void> => {
    try {
